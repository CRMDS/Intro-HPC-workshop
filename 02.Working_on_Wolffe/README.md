...
This script does the following:
- `#SBATCH --partition=ampere24`: Specifies that the job should run in the `ampere24` partition, which is the partition for A100 GPUs and 80Gb RAM.
...
Note that in other HPC systems, you may need to use `#SBATCH --gres=gpu:1` (`gres` for "generic resources") to request a GPU, but on Wolffe, the `--partition=ampere24` option is sufficient to request a GPU.
